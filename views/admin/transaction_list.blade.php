							...
							if(!is_null($property_data_array_json)){
								...
								foreach($property_data_array as $transaction_property_item){
									...
									if(count($property_data)>0){
										...
										$address_text = $property_data[0]->address . ", " . $property_data[0]->city . ", " . $property_data[0]->state . " " . $property_data[0]->zip;
								
										$textDescription .= $current_property_invest_ownership_percentage . "% of <a href='/property/item/" . $current_property_id . "'><u>" . $address_text . "</u></a>; ";
						...
						?>
						  ...
						  <td><?= $transaction_item->id ?></td>
							...
							if( $transaction_item->status == 4 ){
								...
								echo "<a href='/admin/complete-transaction?id=".$transaction_item->id."' class='complete-transaction' data-id='".$transaction_item->id."'style='color:green;'> Mark as Completed </a>";
							...
							if(
								...
								echo "<a href='/admin/cancel-transaction?id=".$transaction_item->id."' style='color:red;'>Cancel</a>";
								
							}
							
							?></td>
                            <td>$<?= number_format((float)$transaction_item->invest_amount, 0, '.', ',') ?></td>
                          </tr>
							<? } ?>
