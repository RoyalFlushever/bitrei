...
jQuery(document).ready(function($) {
	...
	function functionModal() {
		var modalTargetActive = $('.modal-target:target').length;
		if(modalTargetActive) {
			var youtube = $('.modal-target:target').attr('data-youtube');
			if(youtube.length) {
				$('.modal-target:target').next().children('.modal-window').append('<div class="iframe-wrap"><iframe width="854" height="480" src="https://www.youtube.com/embed/' + youtube + '?autoplay=1" frameborder="0" allowfullscreen></iframe></div>');
			}
		}else {
			$('.modal-window .iframe-wrap').remove();
		}
	}
